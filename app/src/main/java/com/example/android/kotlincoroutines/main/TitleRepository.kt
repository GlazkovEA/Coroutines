...
class TitleRepository(private val network: MainNetwork, private val titleDao: TitleDao) {
    ...
    val bitmap: LiveData<Bitmap> get() = _image
    /**
...
suspend fun <T> FakeNetworkCall<T>.await(): T {
    return suspendCoroutine { continuation ->
        addOnResultListener {
            when (result) {
                is FakeNetworkSuccess<T> -> continuation.resume((result as FakeNetworkSuccess<T>).data)
                is FakeNetworkError -> continuation.resumeWithException((result as FakeNetworkError<T>).error)
            }
        }
    }
}